...
import (
	...
	"github.com/b0shka/onvif/device"
	"github.com/b0shka/onvif/networking"
	wsdiscovery "github.com/b0shka/onvif/ws-discovery"
)
...
func (dev *Device) inspect(ctx context.Context, headers map[string]string) (*device.GetCapabilitiesResponse, error) {
	...
	if err = resp.Unmarshal(&capabilitiesResponse); err != nil {
		return nil, fmt.Errorf("GetCapabilitiesResponse err: %w", err)
	}
...
func (dev *Device) updateDeltaTime(ctx context.Context) (time.Duration, error) {
	...
	if err := resp.Unmarshal(&systemDateAndTime); err != nil {
		return 0, fmt.Errorf("GetSystemDateAndTimeResponse err: %w", err)
	}
